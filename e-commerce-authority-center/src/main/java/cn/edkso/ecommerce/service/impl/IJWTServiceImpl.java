...
import cn.edkso.ecommerce.constant.AuthorityConstant;
import cn.edkso.ecommerce.constant.CommonConstant;
import cn.edkso.ecommerce.dao.EcommerceUserDao;
...
public class IJWTServiceImpl implements IJWTService {
    ...
    public String generateToken(String username, String password, int expire) throws Exception {
        ...
        //Token 中塞入对象，即JWT中存储的信息，后端拿到这些信息就可以知道是那根用户在操作
        LoginUserInfo loginUserInfo = new LoginUserInfo(ecommerceUser.getId(), ecommerceUser.getPassword());
        if (expire <= 0){
    ...
    @Override
    public String registryUserAndGenerateToken(UsernameAndPassword usernameAndPassword) throws Exception {
        //先去校验用户名是否存在，如果存在，不能重复注册
        EcommerceUser oldUser = ecommerceUserDao.findByUsername(usernameAndPassword.getUsername());
        if (oldUser != null){
            log.error("username is registryed: [{}]", oldUser.getUsername());
            return null;
        }

        EcommerceUser ecommerceUser = new EcommerceUser();
        BeanUtils.copyProperties(usernameAndPassword, ecommerceUser);
        ecommerceUser.setExtraInfo("{}");

        //注册一个新用户，写一条记录到数据表中
        EcommerceUser save = ecommerceUserDao.save(ecommerceUser);
        log.info("registry user success: [{}], [{}] ", save.getUsername(), save.getPassword());
